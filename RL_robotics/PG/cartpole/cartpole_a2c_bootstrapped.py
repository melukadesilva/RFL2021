## This code uses a2c with bootstrapping to train a cartpole balancing task
import gym
import torch
...
def learn_step(action_log_prob, critic, value, reward, done, next_state, \
               optimizerA, optimizerC, gamma=0.9):
    target_q = reward + gamma * (1 - int(done)) * critic(
...
c_optimizer = torch.optim.Adam(cnet.parameters(), lr=1e-3)
...
for e in count(1):
    ...
    #while True:
        ...
        #next_states.append(nex_states)

        ## do a learning step (online) ##
        learn_step(action_log_prob, cnet, value, reward, is_done, next_state, p_optimizer, c_optimizer)
    ...
    returns = discounted_returns(cnet, next_states, rewards)
    #print(returns)
    action_losses = list()
    critic_losses = list()
